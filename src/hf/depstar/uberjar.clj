...
(defn- manifest
  ...
  (let [#_#_pom-text    (if (and pom-file (.exists (io/file pom-file)))
                      (slurp pom-file)
                      "")
                         ...
                         (when-not (= :thin jar)
                           (str "Main-Class: "
                                (if main-class
                                  ...
                                  "clojure.main")
                                "\n")))
        #_#_properties  (str "#Generated by depstar\n"
                         "#" build-now "\n"
                         "version: " version "\n"
                         "groupId: " group-id "\n"
                         "artifactId: " artifact-id "\n")
        #_#_maven-dir   (str "META-INF/maven/" group-id "/" artifact-id "/")]
    ...
    #_(with-open [is (io/input-stream (.getBytes properties))]
      (when *verbose*
...
(defn run
  [{:keys [#_aot dest jar #_main-class #_no-pom #_^File pom-file suppress verbose]
    :or {jar :uber}
    :as options
    cp-opt :classpath
    #_#_rm-opt :remove}]
  (let [#_#_do-aot    (and aot main-class (not no-pom) (.exists pom-file))
        #_#_tmp-c-dir (when do-aot
        ...
        #_#_cp        (into (cond-> [] #_#_do-aot (conj (str tmp-c-dir)))
                        #_(if rm-opt
                          (remove rm-opt)
                          (map identity))
...
#_(defn uber-main
  [opts args]
  #_(when (some #(#{"-h" "--help"} %) (cons (:dest opts) args))
    (help))
  (let [;; aot        (some #(#{"-C" "--compile"} %) args)
        #_#_main-class (some #(when (#{"-m" "--main"} (first %)) (second %))
                         (partition 2 1 args))
        #_#_no-pom     (some #(#{"-n" "--no-pom"}  %) args)
        #_#_pom-file   (io/file "pom.xml")
        suppress   (some #(#{"-S" "--suppress-clash"} %) args)
        verbose    (some #(#{"-v" "--verbose"} %) args)
        #_#_aot-main   ; sanity check options somewhat:
        (if main-class
          (cond (= :thin (:jar opts))
    ...
    (run (merge opts
                #_aot-main
                {#_#_:no-pom no-pom #_#_:pom-file pom-file
                 :suppress suppress :verbose verbose}))))

#_(defn -main
  [& [destination & args]]
  #_(when-not destination
    (help))
